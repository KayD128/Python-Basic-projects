...
class UssdAtm(Atm):
    ...
    def menu(self):
        top_menu = ["1. Register", "2. Transfer","3. Buy Airtime", "4. Balance enquiry", "5. Back"]
        print("Welcome, pick out of any of the transactions below")
        for option in top_menu:
            print(option)
        user = input("Enter your Choice>> ")
    ...
    def registerUssd(self):
        
        self.oldUssd = input(("Welcome to USSD Banking, please provide your old USSD or bank PIN"))
        self.newUssd = input("Enter your account number here, if you don't have, register with us")


    def buyAirtime(self):
        airtime_amount = int(input("How much airtime would you like to buy>> "))
        self.phone_number = input("Enter the recipient phone number>> ")
        networks = ["1. MTN", "2. Glo", "3. Airtel", "4. 9mobile"]
        for x in networks:
            print(x)
        recipient_network = input("Pick a network>> ")
        if recipient_network == "1" or "2" or "3" or "4":
            print("Thank you for recharging with us")
        else:
